...
for train_index, test_index in kf:#sss.split(x_train, y_train):#kf:
        ...
        num_fold += 1
        '''
        if num_fold == 1 or num_fold == 2:
            continue
        #print(num_fold)
        
        if num_fold == 3:
            yfull_train = list(np.load('trainres2v5.npy'))
            yfull_test = list(np.load('testres2v5.npy'))
            allthresh =  list(np.load('all2tres.npy'))
...
#                      metrics=['accuracy'])
       ...
       # model.save_weights("modelweights.h5")
        model.load_weights("vgg16augmodelweights.h5")
      ...
      #  for i, layer in enumerate(base_model.layers):
       ...
       # model.fit(x_train, y= Y_train,
        ...
        model.fit_generator(datagen.flow(X_train,Y_train,shuffle=True, batch_size=64), validation_data=(X_valid, Y_valid),
                 samples_per_epoch=len(X_train), nb_epoch=8, verbose=1 ,callbacks=callbacks)
        ...
        # model.load_weights('weights.h5')
        model.fit_generator(datagen.flow(X_train,Y_train,shuffle=True, batch_size=64), validation_data=(X_valid, Y_valid),
                 samples_per_epoch=len(X_train), nb_epoch=5, verbose=1 ,callbacks=callbacks)
...
#                p_test = model.predict_generator(datagen.flow(X_valid,shuffle = False, batch_size = 128) , 64)
                p_train = model.predict_generator(datagen.flow(x_train,shuffle = False, batch_size = 128) , 317)
...
#optimize trheshold
opt = np.array(yfull_train[0])
for i in range(1, nfolds):
    opt += np.array(yfull_train[i])
opt /= nfolds



np.save('vgg16opt-224-0.npy', result)


#thres = [0.2] * 17
#thres = (np.asarray(thres1)+np.asarray(thres2))/2
#thres =  np.asarray(allthresh).mean(axis=0)
np.save('vgg16opt-224-0-opt.npy', opt)

thres = optimise_f2_thresholds(y_train, opt)#[0.2] * 17
np.save('vgg16opt-224-0-th.npy', thres)
preds = []
